...
func NewSource(roots []Root, im *Importmap, opts ...BuildOption) (Server, error) {
	...
	normalized, err := normalizeRoots("assets.NewSource", roots)
	if err != nil {
		return nil, err
	}
	for i, r := range normalized {
		if _, err := fs.ReadDir(r.FS, "."); err != nil {
			return nil, fmt.Errorf("assets.NewSource: Roots[%d]: %w; source roots resolve against the process working directory - run from the module root", i, err)
		}
	}
	if im == nil {
		im, err = discoverImportmap("assets.NewSource", normalized)
		if err != nil {
			return nil, err
		}
	}

...
func Compile(srcRoots []Root, publicDir string, opts ...BuildOption) (*Manifest, error) {
	plan, err := planBuild("assets.Compile", srcRoots, nil, opts)
	if err != nil {
		return nil, err
	}
	if err := os.MkdirAll(publicDir, 0o755); err != nil { // #nosec G301 -- served asset, world-readable by design
		return nil, fmt.Errorf("assets.Compile: create publicDir: %w", err)
	}
	cleanupCompileTemps(publicDir)

	stageDir, err := os.MkdirTemp(publicDir, ".assets-build-*.staging")
	if err != nil {
		return nil, fmt.Errorf("assets.Compile: create staging directory: %w", err)
	}
	...
	for _, logical := range plan.order {
		...
		if err := os.MkdirAll(filepath.Dir(dst), 0o755); err != nil { // #nosec G301 -- served asset, world-readable by design
			return nil, fmt.Errorf("assets.Compile: stage mkdir for %s: %w", logical, err)
		}
		if asset.kind == kindJS || asset.kind == kindCSS {
			if err := writeSyncedFile(dst, asset.content, 0o644); err != nil {
				return nil, fmt.Errorf("assets.Compile: stage %s: %w", logical, err)
			}
			continue
		}
		...
		if err != nil {
			return nil, fmt.Errorf("assets.Compile: stream %s: %w", logical, err)
		}
		if hash != asset.outputHash {
			return nil, fmt.Errorf("assets.Compile: source %s changed while compiling", logical)
		}
	}
...
func cleanupCompileTemps(publicDir string) {
	...
	for _, entry := range entries {
		name := entry.Name()
		if entry.IsDir() && strings.HasPrefix(name, ".assets-build-") &&
			strings.HasSuffix(name, ".staging") {
			_ = os.RemoveAll(filepath.Join(publicDir, name))
			continue
		}
		if !entry.IsDir() && strings.HasPrefix(name, ".assets-tmp-") &&
...
func publishCompileStage(stageDir, publicDir string, plan *buildPlan) error {
	for _, logical := range plan.order {
		...
		if _, err := os.Stat(dst); err == nil {
			...
			if err != nil {
				return fmt.Errorf("assets.Compile: compare existing %s: %w", dst, err)
			}
			if !equal {
				return fmt.Errorf("assets.Compile: existing content-addressed file %s has different bytes", dst)
			}
			continue
		} else if !os.IsNotExist(err) {
			return fmt.Errorf("assets.Compile: stat %s: %w", dst, err)
		}
		if err := os.MkdirAll(filepath.Dir(dst), 0o755); err != nil { // #nosec G301 -- served asset, world-readable by design
			return fmt.Errorf("assets.Compile: mkdir for %s: %w", logical, err)
		}
		if err := os.Rename(staged, dst); err != nil {
			return fmt.Errorf("assets.Compile: publish %s: %w", logical, err)
		}
		if err := syncAncestorDirectories(filepath.Dir(dst), publicDir); err != nil {
			return fmt.Errorf("assets.Compile: sync published %s: %w", logical, err)
		}
	}
	...
	if err := os.Rename(stagedManifest, publicManifest); err != nil {
		return fmt.Errorf("assets.Compile: publish manifest: %w", err)
	}

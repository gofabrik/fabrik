...
func (v *Vendor) publishMetadata(lock *VendorLock, next *Importmap) error {
	...
	if err != nil {
		return fmt.Errorf("assets.Vendor: encode transaction: %w", err)
	}
	data = append(data, '\n')
	journal := v.transactionPath()
	if err := atomicWriteFile(journal, data, 0o644); err != nil {
		return fmt.Errorf("assets.Vendor: write transaction journal: %w", err)
	}
...
func (v *Vendor) recoverTransaction() error {
	...
	if err != nil {
		...
		return fmt.Errorf("assets.Vendor: open transaction journal: %w", err)
	}
	...
	if err := json.UnmarshalDecode(dec, &transaction, jsonv1.DefaultOptionsV1(), json.RejectUnknownMembers(true)); err != nil {
		_ = file.Close()
		return fmt.Errorf("assets.Vendor: decode transaction journal: %w", err)
	}
	if err := json.UnmarshalDecode(dec, &struct{}{}, jsonv1.DefaultOptionsV1()); err != io.EOF {
		_ = file.Close()
		if err == nil {
			return fmt.Errorf("assets.Vendor: transaction journal contains multiple JSON values")
		}
		return fmt.Errorf("assets.Vendor: decode transaction journal: %w", err)
	}
	if err := file.Close(); err != nil {
		return fmt.Errorf("assets.Vendor: close transaction journal: %w", err)
	}
	if transaction.Version != 1 || transaction.Lock == nil || transaction.Entries == nil {
		return fmt.Errorf("assets.Vendor: invalid transaction journal")
	}
	if transaction.Lock.Version != 1 && transaction.Lock.Version != vendorLockVersion {
		return fmt.Errorf("assets.Vendor: transaction journal has unsupported lock version %d", transaction.Lock.Version)
	}
	...
	for specifier, pkg := range transaction.Lock.Packages {
		if _, err := vendorRelPath(specifier, pkg.Type); err != nil {
			return fmt.Errorf("assets.Vendor: invalid transaction journal: %w", err)
		}
	...
	if err := transaction.Lock.Verify(v.VendorDir); err != nil {
		return fmt.Errorf("assets.Vendor: recover transaction: %w", err)
	}

...
type stubResolver struct {
	...
	reqs  [][]assets.PackageRequest
}

type contextResolver struct{}

func (contextResolver) Resolve(ctx context.Context, _ []assets.PackageRequest) (*assets.Resolution, error) {
	<-ctx.Done()
	return nil, ctx.Err()
}
...
func localJSPMResolver(server *httptest.Server) *assets.JSPMResolver {
	resolver := assets.NewJSPMResolver(server.Client())
	resolver.AllowHTTP = true
	resolver.AllowPrivateNetwork = true
	return resolver
}

func (s *stubResolver) Resolve(ctx context.Context, reqs []assets.PackageRequest) (*assets.Resolution, error) {
	s.mu.Lock()
	s.reqs = append(s.reqs, append([]assets.PackageRequest(nil), reqs...))
	s.mu.Unlock()
...
func vendoredPath(t *testing.T, vendorDir string, im *assets.Importmap, specifier string) string {
	t.Helper()
...
func TestVendor_MetadataFailureLeavesImportmapUnchanged(t *testing.T) {
	...
	resolver := &stubResolver{
		resolution: &assets.Resolution{Packages: []assets.ResolvedPackage{
...
func TestVendor_RecoversInterruptedMetadataCommit(t *testing.T) {
	...
	journal := filepath.Join(assetsDir, assets.VendorTransactionFilename)
	...
	resolver.resolution = &assets.Resolution{}
	...
	loaded, err := assets.LoadImportmap(importmapPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVendor_RejectsTraversalSpecifiers(t *testing.T) {
	...
	for _, spec := range []string{"../evil", "/evil", "a/../evil", ".", "", `..\evil`} {
		stub := &stubResolver{
			resolution: &assets.Resolution{
				Packages: []assets.ResolvedPackage{
		...
		v := &assets.Vendor{Resolver: stub, VendorDir: vendorDir, Importmap: im}
	...
	im := assets.NewImportmap()
	im.Entries["../evil"] = assets.ImportmapEntry{Version: "1.0.0"}
	v := &assets.Vendor{Resolver: &stubResolver{}, VendorDir: vendorDir, Importmap: im}
...
func TestVendor_RemoveRejectsUnsafeSpecifierBeforeLockOwnership(t *testing.T) {
	...
	importmap.Entries["../evil"] = assets.ImportmapEntry{Version: "1.0.0"}
...
func TestVendor_PruneRejectsMalformedVendoredPathBeforeDeleting(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			key:   "../keep",
			entry: assets.ImportmapEntry{Version: "1.0.0", Type: "js"},
		},
	} {
...
func TestVendor_RejectsCollidingOrPublicMetadataPaths(t *testing.T) {
	...
	for _, vendor := range []*assets.Vendor{
		{
			Resolver: &stubResolver{}, VendorDir: vendorDir, Importmap: assets.NewImportmap(),
...
func TestVendor_RequireWritesFileAndImportmapEntry(t *testing.T) {
	...
	lock, err := assets.LoadVendorLock(filepath.Join(filepath.Dir(vendorDir), assets.VendorLockFilename))
	if err != nil {
		t.Fatal(err)
	}
...
func TestVendor_EnforcesResolutionLimits(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			limits:      assets.DownloadLimits{MaxPackages: 1},
			want:        "limit is 1",
			wantFetches: 0,
		},
		{
			...
			limits:      assets.DownloadLimits{MaxPackageBytes: 4},
			want:        "4-byte limit",
			wantFetches: 1,
		},
		{
			...
			limits:      assets.DownloadLimits{MaxResolutionBytes: 7},
			want:        "7-byte limit",
			wantFetches: 2,
		},
		{
			...
			limits:      assets.DownloadLimits{MaxPackageBytes: 80},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			resolver := &stubResolver{
				resolution: &assets.Resolution{Packages: test.packages},
...
func TestVendor_RejectsPartialProvenanceLock(t *testing.T) {
	...
	lockPath := filepath.Join(filepath.Dir(vendorDir), assets.VendorLockFilename)
	...
	importmap := assets.NewImportmap()
	importmap.Entries["existing"] = assets.ImportmapEntry{Version: "1.0.0", Type: "js"}
	vendor := &assets.Vendor{
		Resolver:  resolver,
		VendorDir: vendorDir,
		Importmap: importmap,
...
func TestVendor_RejectsResolverThatDropsExistingDirectRequirement(t *testing.T) {
	...
	importmap.Entries["existing"] = assets.ImportmapEntry{Version: "2.0.0", Type: "js"}
	resolver.resolution = &assets.Resolution{Packages: []assets.ResolvedPackage{
		{Specifier: "new", Version: "1.0.0", Type: "js", URL: "https://example.com/new.js"},
	}}
...
func TestVendor_ReplacesCompleteGraphAndTracksOwnership(t *testing.T) {
	...
	importmap := assets.NewImportmap()
	vendor := &assets.Vendor{Resolver: resolver, VendorDir: vendorDir, Importmap: importmap}
	if err := vendor.Require(context.Background(), "a", "1.0.0"); err != nil {
		t.Fatal(err)
	}
	lockPath := filepath.Join(filepath.Dir(vendorDir), assets.VendorLockFilename)
	lock, err := assets.LoadVendorLock(lockPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	migrated, err := assets.LoadVendorLock(legacyPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !slices.Equal(lastRequests, []assets.PackageRequest{
...
func TestVendor_RequireOverwritesExistingEntry(t *testing.T) {
	...
	stub.resolution = &assets.Resolution{
		Packages: []assets.ResolvedPackage{
...
func TestVendor_RemoveContextCancelsFullResolution(t *testing.T) {
	...
	if err := vendor.RequirePackages(context.Background(), []assets.PackageRequest{
...
func TestJSPMResolver_RejectsConflictingScopes(t *testing.T) {
	...
	_, err := res.Resolve(context.Background(), []assets.PackageRequest{{Name: "react"}})
	...
	got, err := res.Resolve(context.Background(), []assets.PackageRequest{{Name: "react"}})
	if err != nil {
...
func TestJSPMResolver_ResolvesViaGenerateEndpoint(t *testing.T) {
	...
	res := localJSPMResolver(srv)
	res.BaseURL = srv.URL
	got, err := res.Resolve(context.Background(), []assets.PackageRequest{
	...
	specs := map[string]assets.ResolvedPackage{}
...
func TestVendor_EndToEnd_WithJSPMResolver(t *testing.T) {
	...
	im := assets.NewImportmap()
	v := &assets.Vendor{Resolver: wrapped, VendorDir: vendorDir, Importmap: im}
	if err := v.Require(context.Background(), "react", "18.2.0"); err != nil {
		t.Fatal(err)
	}
...
type fetchRedirectResolver struct {
	Inner   *assets.JSPMResolver
	Prefix  string
	Replace string
}

func (f *fetchRedirectResolver) Resolve(ctx context.Context, reqs []assets.PackageRequest) (*assets.Resolution, error) {
...
func TestVendor_PruneHandlesScopedPackages(t *testing.T) {
	...
	im := assets.NewImportmap()
	im.Entries["@radix-ui/themes"] = assets.ImportmapEntry{Version: "1.0.0"}
...
func (f *failingResolver) Resolve(ctx context.Context, reqs []assets.PackageRequest) (*assets.Resolution, error) {
...
func TestVendor_RequireFetchFailure_LeavesPriorEntriesIntact(t *testing.T) {
	...
	im := assets.NewImportmap()
	v := &assets.Vendor{Resolver: seed, VendorDir: vendorDir, Importmap: im}
	if err := v.Require(context.Background(), "react", "18.0.0"); err != nil {
		t.Fatalf("seed: %v", err)
	}

...
func LoadManifest(publicDir string) (*Manifest, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("assets.LoadManifest: open %s: %w", path, err)
	}
...
func ParseManifest(r io.Reader) (*Manifest, error) {
	...
	if err := json.UnmarshalDecode(jsontext.NewDecoder(r), &m, jsonv1.DefaultOptionsV1()); err != nil {
		return nil, fmt.Errorf("assets.ParseManifest: %w", err)
	}
	if m.Entries == nil {
...
func (m *Manifest) Save(publicDir string) error {
	...
	if err := atomicWriteFile(path, []byte(out.String()), 0o644); err != nil {
		return fmt.Errorf("assets.Manifest.Save: write %s: %w", path, err)
	}
	return nil
}

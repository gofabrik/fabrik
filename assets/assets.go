// Package assets maps logical asset paths to hash-addressed public URLs.
//
// Surface:
//
...
func New(cfg Config) (*Mapper, error) {
	roots, err := normalizeRoots("assets.New", cfg.Roots)
	if err != nil {
		return nil, err
	}
	...
	if manifest != nil && manifest.URLPrefix != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("assets.New: manifest URLPrefix invalid: %w", err)
		}
		if manifestPrefix != prefix {
			return nil, fmt.Errorf("assets.New: URLPrefix mismatch: Config %q vs Manifest %q (the manifest was compiled with a different prefix — recompile or set Config.URLPrefix to match)", prefix, manifestPrefix)
		}
	}
	return &Mapper{
		roots:     roots,

...
func LoadVendorLock(path string) (*VendorLock, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("assets.LoadVendorLock: open %s: %w", path, err)
	}
	...
	if err := json.UnmarshalDecode(dec, &lock, jsonv1.DefaultOptionsV1(), json.RejectUnknownMembers(true)); err != nil {
		return nil, fmt.Errorf("assets.LoadVendorLock: decode %s: %w", path, err)
	}
	if err := json.UnmarshalDecode(dec, &struct{}{}, jsonv1.DefaultOptionsV1()); err != io.EOF {
		if err == nil {
			return nil, fmt.Errorf("assets.LoadVendorLock: decode %s: multiple JSON values", path)
		}
		return nil, fmt.Errorf("assets.LoadVendorLock: decode %s: %w", path, err)
	}
	if lock.Version != 1 && lock.Version != vendorLockVersion {
		return nil, fmt.Errorf("assets.LoadVendorLock: unsupported version %d", lock.Version)
	}
	if lock.Version == 1 {
	...
	for specifier, pkg := range lock.Packages {
		if _, err := vendorRelPath(specifier, pkg.Type); err != nil {
			return nil, fmt.Errorf("assets.LoadVendorLock: %w", err)
		}
...
func (l *VendorLock) Save(path string) error {
	if l == nil {
		return fmt.Errorf("assets.VendorLock.Save: nil lock")
	}
	if l.Version != vendorLockVersion {
		return fmt.Errorf("assets.VendorLock.Save: unsupported version %d", l.Version)
	}
	for specifier, pkg := range l.Packages {
		if _, err := vendorRelPath(specifier, pkg.Type); err != nil {
			return fmt.Errorf("assets.VendorLock.Save: %w", err)
		}
		if err := validateLockedPackage(specifier, pkg); err != nil {
			return fmt.Errorf("assets.VendorLock.Save: %w", err)
		}
	...
	if err := atomicWriteFile(path, data, 0o644); err != nil {
		return fmt.Errorf("assets.VendorLock.Save: write %s: %w", path, err)
	}
	return nil
}
...
func (l *VendorLock) Verify(vendorDir string) error {
	if l == nil {
		return fmt.Errorf("assets.VendorLock.Verify: nil lock")
	}
	if l.Version != vendorLockVersion {
		return fmt.Errorf("assets.VendorLock.Verify: unsupported version %d", l.Version)
	}
	for specifier, pkg := range l.Packages {
		if _, err := vendorRelPath(specifier, pkg.Type); err != nil {
			return fmt.Errorf("assets.VendorLock.Verify: %w", err)
		}
		if err := validateLockedPackage(specifier, pkg); err != nil {
			return fmt.Errorf("assets.VendorLock.Verify: %w", err)
		}
	...
	for specifier, pkg := range l.Packages {
		...
		if err != nil {
			return fmt.Errorf("assets.VendorLock.Verify: open %s: %w", path, err)
		}
		hash := sha256.New()
		size, copyErr := io.Copy(hash, file)
		closeErr := file.Close()
		if copyErr != nil {
			return fmt.Errorf("assets.VendorLock.Verify: read %s: %w", path, copyErr)
		}
		if closeErr != nil {
			return fmt.Errorf("assets.VendorLock.Verify: close %s: %w", path, closeErr)
		}
		digest := hex.EncodeToString(hash.Sum(nil))
		if size != pkg.Size || digest != pkg.SHA256 {
			return fmt.Errorf("assets.VendorLock.Verify: %s does not match locked size and SHA-256", specifier)
		}
	}
	return nil
}

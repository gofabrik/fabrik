// Package directive integrates assets with the fabrik code generator.
package directive
...
const assetsPath = "github.com/gofabrik/fabrik/assets"

const compiledPath = "*" + assetsPath + ".Compiled"

const serverPath = assetsPath + ".Server"

// urlPrefix is fabrik's asset URL root; standalone users pick their
// own via assets.WithURLPrefix.
...
func (*Assets) Meta() gen.Meta {
	return gen.Meta{
		...
		Doc: "**`//fabrik:assets [dir=assets]`**\n\n" +
			...
			"Declaring `type AssetsConfig = assets.Options` " +
			...
			"as before - and binds `assets.Server` instead of " +
			"`*assets.Compiled`.\n\n" +
			"```go\n//fabrik:assets\n//go:embed all:assets\nvar Assets embed.FS\n```",
...
func (as *Assets) Emit(n any, g *gen.Gen) diag.Diagnostics {
	...
	if as.config != nil {
		...
		if ok && section != "assets" {
			var ds diag.Diagnostics
			ds.Error(pos, fmt.Sprintf("assets.Options must be declared under //fabrik:config assets, not section %q", section),
...
func (as *Assets) Validate(*gen.Gen) diag.Diagnostics {
	...
	if !collided {
		roots := make([]assets.Root, len(decls))
		for i, d := range decls {
			roots[i] = assets.Root{FS: as.treeFS(d.srcDir), Dir: d.dir}
		}
		if err := assets.Check(roots, nil); err != nil {
...
func (as *Assets) MissingHint(ty types.Type) (string, bool) {
	...
	case assetsPath + ".Compiled":
		if as.switchMode {
			return "with a source/compiled asset switch only assets.Server is injectable; take assets.Server", true
		}
		return "compiled assets are injected as pointers; take *assets.Compiled", true
	case "*" + assetsPath + ".Compiled":
		if as.switchMode {
			return "with a source/compiled asset switch only assets.Server is injectable; take assets.Server", true
		}

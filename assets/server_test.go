...
var _ assets.Server = (*assets.Compiled)(nil)
...
func sourceServer(t *testing.T, dir string) assets.Server {
	t.Helper()
	rt, err := assets.NewSource([]assets.Root{{FS: os.DirFS(dir)}}, nil)
	if err != nil {
		t.Fatal(err)
	}
	return rt
...
func TestNewSource_RejectsMissingRoot(t *testing.T) {
	_, err := assets.NewSource([]assets.Root{
...
func TestNewSource_RejectsUnreadableRoot(t *testing.T) {
	...
	_, err := assets.NewSource([]assets.Root{{FS: os.DirFS(locked)}}, nil)
	if err == nil {
...
func TestNewSource_SnapshotsImportmap(t *testing.T) {
	...
	im.Entries["app"] = assets.ImportmapEntry{Path: "app.js"}
	rt, err := assets.NewSource([]assets.Root{{FS: os.DirFS(dir)}}, im)
	if err != nil {
		t.Fatal(err)
	}
	im.Entries["late"] = assets.ImportmapEntry{Path: "app.js"}
...
func TestNewSource_URLPrefixParityWithCompiled(t *testing.T) {
	dir := t.TempDir()
	writeAsset(t, dir, "app.js", "export {}")
	roots := []assets.Root{{FS: os.DirFS(dir)}}

	src, err := assets.NewSource(roots, nil, assets.WithURLPrefix("/static/"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for name, rt := range map[string]assets.Server{"source": src, "compiled": compiled} {
...
func TestCompiled_ImportmapCSPSources(t *testing.T) {
	dir := t.TempDir()
	writeAsset(t, dir, "app.js", "export {}")
	c, err := assets.Build([]assets.Root{{FS: os.DirFS(dir)}}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimeConfig_Mode(t *testing.T) {
	cases := []struct {
		...
		{"", assets.KindCompiled, false},
		{"compiled", assets.KindCompiled, false},
		{"source", assets.KindSource, false},
		{"bundler", "", true},
	}
	for _, c := range cases {
		got, err := assets.Options{Kind: c.kind}.Mode()
		if c.err {
			if err == nil {

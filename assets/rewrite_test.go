...
func compileAndRead(t *testing.T, src fstest.MapFS, logical string) (string, *assets.Manifest) {
	t.Helper()
	dir := t.TempDir()
...
`)},
	...
	if _, err := assets.Compile([]assets.Root{{FS: src}}, t.TempDir()); err != nil {
...
func TestCompile_RewritesNoSubstitutionTemplateImport(t *testing.T) {
	...
	if _, err := assets.Compile([]assets.Root{{FS: missing}}, t.TempDir()); err == nil ||
...
func TestCompile_TransitiveHashChangeOnDepUpdate(t *testing.T) {
	...
	dir1 := t.TempDir()
	dir2 := t.TempDir()
	m1, err := assets.Compile([]assets.Root{{FS: v1}}, dir1)
	if err != nil {
		t.Fatal(err)
	}
	m2, err := assets.Compile([]assets.Root{{FS: v2}}, dir2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_SupportsImportCycle(t *testing.T) {
	...
	repeated, err := assets.Compile([]assets.Root{{FS: src}}, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_CycleChangeInvalidatesComponentAndImporters(t *testing.T) {
	...
	first, err := assets.Compile([]assets.Root{{FS: version("1")}}, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	second, err := assets.Compile([]assets.Root{{FS: version("2")}}, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_CycleDigestIncludesURLPrefix(t *testing.T) {
	...
	defaultManifest, err := assets.Compile([]assets.Root{{FS: src}}, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	customDir := t.TempDir()
	customManifest, err := assets.Compile(
		[]assets.Root{{FS: src}},
		customDir,
...
func TestCompile_CustomURLPrefix(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile(
		[]assets.Root{{FS: src}}, dir,

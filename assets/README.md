...
compiled, err := assets.Build([]assets.Root{
	{FS: assetFS, Dir: "assets"},
}, nil)
if err != nil {
	log.Fatal(err)
}
...
`cmd/assets` is the vendoring CLI - it downloads a package and
its transitive dependencies from jspm.io into `<dir>/vendor/` and
registers them in `<dir>/importmap.json`:

```sh
go install github.com/gofabrik/fabrik/assets/cmd/assets@latest

assets require -dir web/assets htmx.org@2.0.3
assets remove  -dir web/assets htmx.org
assets prune   -dir web/assets
```
...
remain as harmless unreferenced bytes until `assets prune`.

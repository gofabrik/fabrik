...
func LoadImportmap(path string) (*Importmap, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("assets.LoadImportmap: open %s: %w", path, err)
	}
...
func ParseImportmap(r io.Reader) (*Importmap, error) {
	...
	if err := json.UnmarshalDecode(jsontext.NewDecoder(r), &entries, jsonv1.DefaultOptionsV1(), json.RejectUnknownMembers(true)); err != nil {
		return nil, fmt.Errorf("assets.ParseImportmap: %w", err)
	}
	if entries == nil {
		entries = map[string]ImportmapEntry{}
	}
...
func (im *Importmap) Save(path string) error {
	...
	if err := atomicWriteFile(path, []byte(out.String()), 0o644); err != nil {
		return fmt.Errorf("assets.Importmap.Save: write %s: %w", path, err)
	}
	return nil
}
...
func (im *Importmap) modulePreloadLinksWithOptions(m *Mapper, opts RenderOptions) (string, error) {
	if m == nil {
		return "", fmt.Errorf("assets.Importmap.ModulePreloadLinks: nil Mapper")
	}
	if err := im.validateEntrypoints(opts.Entrypoints); err != nil {
		return "", fmt.Errorf("assets.Importmap.ModulePreloadLinks: %w", err)
	}

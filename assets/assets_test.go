...
func mustMapper(t *testing.T, cfg assets.Config) *assets.Mapper {
	t.Helper()
	m, err := assets.New(cfg)
	if err != nil {
		t.Fatal(err)
	}
	return m
...
func TestNew_RejectsNoRoots(t *testing.T) {
	if _, err := assets.New(assets.Config{}); err == nil {
...
func TestNew_RejectsBadMount(t *testing.T) {
	...
	for _, mount := range cases {
		_, err := assets.New(assets.Config{
			Roots: []assets.Root{{FS: fstest.MapFS{}, MountAt: mount}},
		})
		if err == nil {
...
func TestNew_DefaultsURLPrefix(t *testing.T) {
	m := mustMapper(t, assets.Config{
		Roots: []assets.Root{{FS: fstest.MapFS{}}},
	})
...
func TestNew_AcceptsMatchingURLPrefix(t *testing.T) {
	...
	if _, err := assets.New(assets.Config{
		Roots:     []assets.Root{{FS: src}},
		URLPrefix: "/static/",
		Manifest:  manifest,
	}); err != nil {
...
func TestNew_LenientWhenManifestURLPrefixEmpty(t *testing.T) {
	...
	src := fstest.MapFS{}
	manifest := &assets.Manifest{Entries: map[string]string{
...
func TestNew_SnapshotsManifest(t *testing.T) {
	...
	if _, err := m.Asset("late.js"); !errors.Is(err, assets.ErrAssetNotFound) {
...
func TestCompile_PersistsURLPrefixInManifest(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile([]assets.Root{{FS: src}}, dir,
		assets.WithURLPrefix("/static/v2/"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAsset_HashesContentInDevMode(t *testing.T) {
	...
	m := mustMapper(t, assets.Config{Roots: []assets.Root{{FS: fs}}})
...
func TestAsset_FirstRootWins(t *testing.T) {
	...
	m := mustMapper(t, assets.Config{
		Roots: []assets.Root{{FS: user}, {FS: lib}},
	})
	...
	libOnly := mustMapper(t, assets.Config{
		Roots: []assets.Root{{FS: lib}},
	})
...
func TestAsset_MountAtNamespacesRoot(t *testing.T) {
	...
	m := mustMapper(t, assets.Config{
		Roots: []assets.Root{{FS: libAssets, MountAt: "jobs"}},
	})
	...
	if _, err := m.Asset("styles.css"); !errors.Is(err, assets.ErrAssetNotFound) {
...
func TestAsset_NotFoundForMissingPath(t *testing.T) {
	...
	if _, err := m.Asset("missing.js"); !errors.Is(err, assets.ErrAssetNotFound) {
...
func TestAsset_RejectsTraversal(t *testing.T) {
	m := mustMapper(t, assets.Config{
		Roots: []assets.Root{{FS: fstest.MapFS{"app.js": {Data: []byte("x")}}}},
	})
	for _, logical := range []string{
		...
		if _, err := m.Asset(logical); !errors.Is(err, assets.ErrAssetNotFound) {
...
func TestAsset_ProdModeMissingFromManifestIsError(t *testing.T) {
	...
	if _, err := m.Asset("app.js"); !errors.Is(err, assets.ErrAssetNotFound) {
		t.Errorf("err = %v, want ErrAssetNotFound", err)
	}
}
...
func TestHandler_PrefersExactHashShapedLogicalPath(t *testing.T) {
	hashShaped := "foo-" + strings.Repeat("a", assets.HashLength) + ".js"
	for name, files := range map[string]fstest.MapFS{
		...
		t.Run(name, func(t *testing.T) {
			m := mustMapper(t, assets.Config{Roots: []assets.Root{{FS: files}}})
			rec := httptest.NewRecorder()
...
func TestHandler_404ForDirectoryPaths(t *testing.T) {
	...
	if _, err := m.Asset("css"); !errors.Is(err, assets.ErrAssetNotFound) {

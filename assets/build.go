...
func Build(roots []Root, im *Importmap, opts ...BuildOption) (*Compiled, error) {
	plan, err := planBuild("assets.Build", roots, im, opts)
	if err != nil {
		return nil, err
	}
	...
	for _, logical := range plan.order {
		...
		if asset.kind == kindJS || asset.kind == kindCSS {
			...
			if err != nil {
				return nil, fmt.Errorf("assets.Build: snapshot %s: %w", logical, err)
			}
			if int64(len(content)) != asset.size || hashContent(content) != asset.outputHash {
				return nil, fmt.Errorf("assets.Build: source %s changed while snapshotting", logical)
			}
...
func Check(roots []Root, im *Importmap, opts ...BuildOption) error {
	_, err := planBuild("assets.Check", roots, im, opts)
	return err
}

...
func TestCompile_UsesSamePlanAsBuild(t *testing.T) {
	...
	compiled, err := assets.Build(roots, nil, assets.WithURLPrefix("/static/"))
	if err != nil {
		t.Fatal(err)
	}
	manifest, err := assets.Compile(roots, t.TempDir(), assets.WithURLPrefix("/static/"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_ValidatesDiscoveredImportmap(t *testing.T) {
	...
	_, err := assets.Compile([]assets.Root{{FS: src}}, t.TempDir())
...
func TestCompile_PersistsPreloadDependencies(t *testing.T) {
	...
	dir := t.TempDir()
	if _, err := assets.Compile([]assets.Root{{FS: src}}, dir); err != nil {
		t.Fatal(err)
	}
	...
	delete(src, "app.js")
	delete(src, "dep.js")
	mapper, err := assets.New(assets.Config{
		Roots:    []assets.Root{{FS: src}},
		Manifest: manifest,
	})
	if err != nil {
...
func TestCompile_RejectsNoRoots(t *testing.T) {
	if _, err := assets.Compile(nil, t.TempDir()); err == nil {
		t.Fatal("expected error for empty Roots")
	}
}

func TestCompile_RejectsNilFS(t *testing.T) {
	_, err := assets.Compile(
		[]assets.Root{{FS: nil}}, t.TempDir(),
	)
	if err == nil {
...
func TestCompile_HashesEveryFile(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile([]assets.Root{{FS: srcFS}}, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_MountAtPrefixesLogicalAndOutputPath(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile([]assets.Root{
...
func TestCompile_FirstRootWinsShadowing(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile([]assets.Root{{FS: user}, {FS: lib}}, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_PersistsManifestToDisk(t *testing.T) {
	...
	dir := t.TempDir()
	if _, err := assets.Compile([]assets.Root{{FS: srcFS}}, dir); err != nil {
		t.Fatal(err)
	}
	got, err := assets.LoadManifest(dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_IdempotentForUnchangedInput(t *testing.T) {
	...
	srcFS := fstest.MapFS{"app.js": {Data: []byte("x")}}
	dir := t.TempDir()
	first, err := assets.Compile([]assets.Root{{FS: srcFS}}, dir)
	if err != nil {
		t.Fatal(err)
	}
	second, err := assets.Compile([]assets.Root{{FS: srcFS}}, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_HashSuffixedSourceWithoutShadowIsFine(t *testing.T) {
	...
	manifest, err := assets.Compile([]assets.Root{{FS: src}}, t.TempDir())
	if err != nil {
...
func TestCompile_StreamingPathHashesLargeNonJSCSSFile(t *testing.T) {
	...
	dir := t.TempDir()
	manifest, err := assets.Compile([]assets.Root{{FS: src}}, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_RemovesStaleTempFilesFromPriorCrashedRun(t *testing.T) {
	...
	stale := filepath.Join(dir, ".assets-tmp-stale-deadbeef.tmp")
...
func TestCompile_StaleTempCleanupIsNarrow(t *testing.T) {
	// Only assets temp files are removed.
	dir := t.TempDir()
	if err := os.MkdirAll(dir, 0o750); err != nil {
		t.Fatal(err)
	}
	keeper := filepath.Join(dir, ".assets-readme.tmp")
	...
	if _, err := os.Stat(keeper); err != nil {
		t.Errorf(".assets-readme.tmp (not ours) was GC'd: %v", err)
	}
}
...
func TestCompile_PublishFailureKeepsPreviousManifest(t *testing.T) {
	...
	oldManifest, err := assets.Compile([]assets.Root{{FS: oldSource}}, dir)
	if err != nil {
		t.Fatal(err)
	}
	...
	probeDir := t.TempDir()
	newManifest, err := assets.Compile([]assets.Root{{FS: newSource}}, probeDir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompile_StreamingPathCleansUpTempOnCollision(t *testing.T) {
	...
	// accumulates orphaned ".assets-tmp-*.tmp" entries.
	//
	...
	dir := t.TempDir()
	_, err := assets.Compile([]assets.Root{{FS: src}}, dir)
	if err == nil {
		t.Fatal("expected collision error")
	}
	...
	for _, e := range entries {
		if strings.HasPrefix(e.Name(), ".assets-tmp-") {
...
func TestCompile_DifferentContentProducesDifferentHash(t *testing.T) {
	...
	dir1 := t.TempDir()
	dir2 := t.TempDir()
	m1, _ := assets.Compile([]assets.Root{{FS: a}}, dir1)
	m2, _ := assets.Compile([]assets.Root{{FS: b}}, dir2)

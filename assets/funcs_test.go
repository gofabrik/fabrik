...
func newMapper(t *testing.T, src fstest.MapFS) *assets.Mapper {
	t.Helper()
...
func execTemplate(t *testing.T, src string, m *assets.Mapper, im *assets.Importmap) (string, error) {
	t.Helper()
	tpl, err := template.New("t").Funcs(assets.FuncMap(m, im)).Parse(src)
	if err != nil {
...
func TestFuncMap_ImportmapOutputIsNotEscaped(t *testing.T) {
	// importmap returns template.HTML so html/template preserves tags.
...
func TestFuncMap_SnapshotsImportmapAtBinding(t *testing.T) {
	...
	render := assets.FuncMap(m, im)["importmap"].(func(...string) (template.HTML, error))
	delete(im.Entries, "app")
	im.Entries["other"] = assets.ImportmapEntry{Path: "other.js", Entrypoint: true}
...
func TestFuncMap_BoundSnapshotIsConcurrentWithBuilderMutation(t *testing.T) {
	...
	wg.Go(func() {
		for range 500 {
			im.Entries["app"] = assets.ImportmapEntry{Path: "other.js", Entrypoint: true}
			im.Entries["app"] = assets.ImportmapEntry{Path: "app.js", Entrypoint: true}
		}
	})
	for range 8 {
...
func TestFuncMap_DevToProdSwap(t *testing.T) {
	...
	prod, err := assets.New(assets.Config{
		Roots:    []assets.Root{{FS: src}},
		Manifest: manifest,
	})
	if err != nil {

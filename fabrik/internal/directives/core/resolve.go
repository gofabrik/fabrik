...
func resolveArgs(g *gen.Gen, cfg *cfgdir.Config, params []param,
	...
	for _, pr := range params {
		...
		if len(eds) == 0 {
			msg, help, final := reject(pr)
			if !final {
				help = missingHelp(g, cfg, pr.t, help)
			}

...
import (
	...
	jobsmysql "github.com/gofabrik/fabrik/jobs/mysql"
...
func openClosedStore(t *testing.T) (*jobsmysql.Store, error) {
	t.Helper()
	db := openClosedDB(t)
	s, err := jobsmysql.New(db, jobsmysql.Options{})
	if err != nil {
		t.Fatal(err)
	}
	db.Close()
...
func TestClosedDB_NilDB(t *testing.T) {
	_, err := jobsmysql.New(nil, jobsmysql.Options{})
...
func TestClosedDB_ErrorPassthrough(t *testing.T) {
	...
	t.Run("createSchema", func(t *testing.T) {
		db := openClosedDB(t)
		db.Close()
		_, err := jobsmysql.New(db, jobsmysql.Options{AutoCreate: true})

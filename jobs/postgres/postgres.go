...
//
// Use pgx as the driver, ReadCommitted transaction isolation, and BYTEA for
// binary columns: "postgres://user:pass@host/db?sslmode=disable".
package postgres
...
// byteKey uses []byte so arbitrary unique keys retain their BYTEA encoding.
func byteKey(s string) any { return []byte(s) }
...
func (s *Store) insertTx(ctx context.Context, tx *sql.Tx, now time.Time, js []jobs.Job) ([]jobs.InsertResult, error) {
	now = now.UTC()
	const liveHolder = "SELECT id FROM jobs WHERE kind=$1 AND handler_id=$2 AND unique_key=$3 AND state NOT IN (" + terminalStates + ") LIMIT 1"
	// Lock the post-conflict lookup against a concurrent insert.
	const conflictHolder = "SELECT id FROM jobs WHERE kind=$1 AND handler_id=$2 AND unique_key=$3 AND state NOT IN (" + terminalStates + ") LIMIT 1 FOR UPDATE"
...
func (s *Store) insertOne(ctx context.Context, tx *sql.Tx, now time.Time, j jobs.Job, liveHolder, conflictHolder string) (jobs.InsertResult, error) {
	...
	for range 3 {
		if j.UniqueKey != "" {
			var existing string
			err := tx.QueryRowContext(ctx, liveHolder, j.Kind, j.HandlerID, byteKey(j.UniqueKey)).Scan(&existing)
			if err == nil {
...
VALUES ($1,$2,$3,$4,$5,$6,$7,0,$8,$9,$10,$11,$12,$13,$14,'',$15,$16) ON CONFLICT DO NOTHING`,
			id, j.Kind, j.HandlerID, j.Payload, j.Queue, j.Priority, string(state), j.MaxAttempts,
			nanos(j.AvailableAt), j.TimeoutMs, int(j.OnTimeout), j.BackoffSpec, byteKey(j.UniqueKey),
		...
		if n, err := res.RowsAffected(); err != nil {
			...
			var existing string
			e2 := tx.QueryRowContext(ctx, conflictHolder, j.Kind, j.HandlerID, byteKey(j.UniqueKey)).Scan(&existing)
			if e2 == sql.ErrNoRows {
...
func (s *Store) Claim(ctx context.Context, req jobs.ClaimRequest) ([]jobs.ClaimedJob, error) {
	...
	var args []any
	arg := func() string { return "$" + strconv.Itoa(len(args)) }
	var b strings.Builder
	args = append(args, nanos(req.Now))
	b.WriteString("SELECT " + jobCols + " FROM jobs WHERE state IN ('available','pending') AND available_at <= " + arg())
	...
	for k := range req.Handlers {
		...
		b.WriteString("(" + arg())
		args = append(args, k.HandlerID)
		b.WriteString("," + arg() + ")")
	}
	b.WriteString(")")
	args = append(args, fetch)
	b.WriteString(" ORDER BY priority DESC, available_at ASC, id ASC LIMIT " + arg())
	...
	const claimSQL = `UPDATE jobs SET state='running', locked_by=$1, locked_until=$2, updated_at=$3
			 WHERE id=$4 AND state IN ('available','pending') AND locked_by=''`
...
func (s *Store) Heartbeat(ctx context.Context, jobID, workerID string, now, until time.Time) (bool, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT cancel_requested FROM jobs WHERE id=$1 AND state='running' AND locked_by=$2",
	...
	res, err := s.db.ExecContext(ctx,
		"UPDATE jobs SET locked_until=$1, updated_at=$2 WHERE id=$3 AND locked_by=$4 AND state='running'",
...
func (s *Store) Complete(ctx context.Context, jobID, workerID string, now time.Time, o jobs.Outcome) (jobs.State, error) {
	...
	err = tx.QueryRowContext(ctx,
		"SELECT state, cancel_requested, unique_key, kind, handler_id FROM jobs WHERE id=$1 AND locked_by=$2",
	...
	if record {
		if cancelReq != 0 && o.AttemptState == "" {
			var cur int
			if err := tx.QueryRowContext(ctx, "SELECT attempt FROM jobs WHERE id=$1", jobID).Scan(&cur); err != nil {
				return "", err
			}
			attemptNum = cur + 1
...
VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
	...
	res, err := tx.ExecContext(ctx, `UPDATE jobs SET state=$1, attempt=$2, error=$3, available_at=CASE WHEN $4>0 THEN $5 ELSE available_at END,
locked_by='', locked_until=0, cancel_requested=0, updated_at=GREATEST(updated_at, $6) WHERE id=$7 AND locked_by=$8 AND state='running'`,
...
func (s *Store) SweepExpired(ctx context.Context, now time.Time) (int, error) {
	...
	rows, err := tx.QueryContext(ctx,
		"SELECT id, attempt, max_attempts, locked_by, locked_until, cancel_requested FROM jobs WHERE state='running' AND locked_until>0 AND locked_until<=$1",
		nanos(now))
	if err != nil {
		return 0, err
	}
	...
	count := 0
	const reclaimSQL = `UPDATE jobs SET state=$1, attempt=$2, error=$3, available_at=$4, locked_by='', locked_until=0, cancel_requested=0, updated_at=$5
			 WHERE id=$6 AND state='running' AND locked_by=$7 AND locked_until=$8`
...
func (s *Store) Get(ctx context.Context, id string) (*jobs.JobInfo, error) {
	...
	info, _, err := scanJob(s.db.QueryRowContext(ctx, "SELECT "+jobCols+" FROM jobs WHERE id=$1", id))
	if err == sql.ErrNoRows {
...
func (s *Store) List(ctx context.Context, f jobs.ListFilter) ([]jobs.JobInfo, string, error) {
	...
	var args []any
	arg := func() string { return "$" + strconv.Itoa(len(args)) }
	inList := func(col string, vals []string) string {
		var c strings.Builder
		c.WriteString(" AND " + col + " IN (")
		for i, v := range vals {
			if i > 0 {
				c.WriteString(",")
			}
	...
	if len(f.Queues) > 0 {
		b.WriteString(inList("queue", f.Queues))
	}
	if len(f.Kinds) > 0 {
		b.WriteString(inList("kind", f.Kinds))
	}
	if len(f.States) > 0 {
		...
		b.WriteString(inList("state", states))
	}
	if f.Cursor != "" {
		args = append(args, nanos(cursorTime))
		first := arg()
		args = append(args, nanos(cursorTime))
		second := arg()
		args = append(args, cursorID)
		b.WriteString(" AND (created_at > " + first + " OR (created_at = " + second + " AND id > " + arg() + "))")
	}
	args = append(args, limit+1)
	b.WriteString(" ORDER BY created_at ASC, id ASC LIMIT " + arg())
...
func (s *Store) ListAttempts(ctx context.Context, jobID string, afterAttempt, limit int) ([]jobs.Attempt, error) {
	rows, err := s.db.QueryContext(ctx,
		"SELECT id,job_id,attempt,worker_id,state,error,started_at,finished_at FROM job_attempts WHERE job_id=$1 AND attempt>$2 ORDER BY attempt ASC LIMIT $3",
...
func (s *Store) retryOnce(ctx context.Context, jobID string, now time.Time) (bool, error) {
	...
	const liveOther = "SELECT id FROM jobs WHERE kind=$1 AND handler_id=$2 AND unique_key=$3 AND id<>$4 AND state NOT IN (" + terminalStates + ") LIMIT 1"
	var state, uniqueKey, kind, handler string
	var attempt, maxAtt int
	err = tx.QueryRowContext(ctx,
		"SELECT state, unique_key, kind, handler_id, attempt, max_attempts FROM jobs WHERE id=$1",
	...
	if uniqueKey != "" {
		var holder string
		err := tx.QueryRowContext(ctx, liveOther, kind, handler, byteKey(uniqueKey), jobID).Scan(&holder)
		if err == nil {
	...
	res, err := tx.ExecContext(ctx,
		"UPDATE jobs SET state='available', max_attempts=$1, available_at=$2, error='', locked_by='', locked_until=0, cancel_requested=0, updated_at=$3 WHERE id=$4 AND state IN ('failed','discarded','cancelled')",
	...
	if err != nil {
		if uniqueKey != "" {
			...
			var holder string
			if e := s.db.QueryRowContext(ctx, liveOther, kind, handler, byteKey(uniqueKey), jobID).Scan(&holder); e == nil {
				return false, &jobs.DuplicateError{ExistingID: holder, Kind: kind, HandlerID: handler, UniqueKey: uniqueKey}
...
func (s *Store) Cancel(ctx context.Context, jobID string, now time.Time) (bool, error) {
	...
	defer tx.Rollback() //nolint:errcheck // rollback is best-effort cleanup after commit or an earlier error
	res, err := tx.ExecContext(ctx,
		"UPDATE jobs SET state='cancelled', updated_at=$1 WHERE id=$2 AND state IN ('pending','available')",
		nanos(now), jobID)
	if err != nil {
		return false, err
	}
	...
	res, err = tx.ExecContext(ctx,
		"UPDATE jobs SET cancel_requested=1, updated_at=$1 WHERE id=$2 AND state='running'",
		nanos(now), jobID)
	if err != nil {
		return false, err
	}
	...
	var state string
	err = tx.QueryRowContext(ctx, "SELECT state FROM jobs WHERE id=$1", jobID).Scan(&state)
	if err == sql.ErrNoRows {
		return false, jobs.ErrNotFound
	}
	if err != nil {
...
func (s *Store) Delete(ctx context.Context, jobID string) error {
	...
	defer tx.Rollback() //nolint:errcheck // rollback is best-effort cleanup after commit or an earlier error
	res, err := tx.ExecContext(ctx, "DELETE FROM jobs WHERE id=$1 AND state<>'running'", jobID)
	if err != nil {
		return err
	}
	if n, err := res.RowsAffected(); err != nil {
		...
		var state string
		if e := tx.QueryRowContext(ctx, "SELECT state FROM jobs WHERE id=$1", jobID).Scan(&state); e == sql.ErrNoRows {
			return jobs.ErrNotFound
		} else if e != nil {
			return e
		}
	...
	if _, err := tx.ExecContext(ctx, "DELETE FROM job_attempts WHERE job_id=$1", jobID); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (s *Store) RetireWorker(ctx context.Context, workerID string) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM job_workers WHERE id=$1", workerID)
	return err
}
...
func (s *Store) SweepStaleWorkers(ctx context.Context, olderThan time.Time) (int, error) {
	res, err := s.db.ExecContext(ctx, "DELETE FROM job_workers WHERE last_seen_at>0 AND last_seen_at<$1", nanos(olderThan))
	if err != nil {
		return 0, err
	}
...
func (s *Store) UpsertSchedule(ctx context.Context, row jobs.ScheduleRow) error {
	...
	err = tx.QueryRowContext(ctx, "SELECT spec, next_run_at, last_run_at FROM job_schedules WHERE sched_group=$1 AND name=$2",
...
VALUES ($1,$2,$3,$4,$5,$6,$7,NULL,$8)`,
	...
	_, err = tx.ExecContext(ctx, `UPDATE job_schedules SET kind=$1, spec=$2, payload=$3, options_json=$4, next_run_at=$5, updated_at=$6
WHERE sched_group=$7 AND name=$8`,
...
func (s *Store) DeleteSchedule(ctx context.Context, group, name string) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM job_schedules WHERE sched_group=$1 AND name=$2", group, name)
	return err
}

func (s *Store) ListSchedules(ctx context.Context, group string) ([]jobs.ScheduleRow, error) {
	return s.querySchedules(ctx, "SELECT sched_group,name,kind,spec,payload,options_json,next_run_at,last_run_at,updated_at FROM job_schedules WHERE sched_group=$1 ORDER BY name", group)
}

func (s *Store) DueSchedules(ctx context.Context, group string, now time.Time) ([]jobs.ScheduleRow, error) {
	return s.querySchedules(ctx, "SELECT sched_group,name,kind,spec,payload,options_json,next_run_at,last_run_at,updated_at FROM job_schedules WHERE sched_group=$1 AND next_run_at<=$2 ORDER BY name", group, nanos(now))
}
...
// fireCAS elects one scheduler per tick and includes a NULL last run in the CAS.
func (s *Store) fireCAS(ctx context.Context, tx *sql.Tx, f jobs.ScheduleFire) (bool, error) {
	...
	res, err := tx.ExecContext(ctx,
		"UPDATE job_schedules SET last_run_at=$1, next_run_at=$2, updated_at=$3 WHERE sched_group=$4 AND name=$5 AND last_run_at IS NOT DISTINCT FROM $6",
	...
	if n, err := res.RowsAffected(); err != nil {
		...
		var exists int
		if e2 := tx.QueryRowContext(ctx, "SELECT 1 FROM job_schedules WHERE sched_group=$1 AND name=$2", f.Group, f.Name).Scan(&exists); e2 == sql.ErrNoRows {
			return false, jobs.ErrNotFound
		} else if e2 != nil {
			return false, e2
		}

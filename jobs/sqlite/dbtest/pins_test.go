...
func openClosedStore(t *testing.T) (*jobssqlite.Store, error) {
	t.Helper()
	db := openClosedDB(t)
	s, err := jobssqlite.New(db, jobssqlite.Options{})
	if err != nil {
		t.Fatal(err)
	}
	db.Close()
...
func TestClosedDB_NilDB(t *testing.T) {
	_, err := jobssqlite.New(nil, jobssqlite.Options{})
...
func TestClosedDB_ErrorPassthrough(t *testing.T) {
	...
	t.Run("createSchema", func(t *testing.T) {
		db := openClosedDB(t)
		db.Close()
		_, err := jobssqlite.New(db, jobssqlite.Options{AutoCreate: true})
	...
	t.Run("insertTx", func(t *testing.T) {
		...
		txDB, err := sql.Open("sqlite", "file::memory:")
		if err != nil {
			t.Fatal(err)
		}
		defer txDB.Close() //nolint:errcheck // best-effort cleanup
		tx, err := txDB.Begin()
		if err != nil {
			t.Fatal(err)
		}

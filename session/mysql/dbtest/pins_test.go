...
func openClosedStore(t *testing.T) *sessionmysql.Store {
	t.Helper()
	db := openClosedDB(t)
	s, err := sessionmysql.New(db, sessionmysql.Options{})
	if err != nil {
		t.Fatal(err)
	}
	db.Close()
...
func TestClosedDB_ErrorWraps(t *testing.T) {
	...
	t.Run("createSchema", func(t *testing.T) {
		db := openClosedDB(t)
		db.Close()
		_, err := sessionmysql.New(db, sessionmysql.Options{AutoCreate: true})
